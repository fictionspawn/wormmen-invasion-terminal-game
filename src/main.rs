fn main() {
    ...
    while game == true {
        ...
        if y == 0 {
            ...
            if x < 1 {
                println!("Ther's a solid vrick wall to Frenk's left.");
            }
            if x > 13 {
            ...
            if x == 1 {
                ...
                if choice.contains("s") {
                    if inventory.contains("lantern") {
                        println!("Frenk is in a dark cellar, but his lantern guides the path.\nSomeweird sounds can be heard from the tunnel to the left.\nTo the right there is a hole in the ground and a rock wall.");
                        y = -1;
                        x = 0;
                    } else {
